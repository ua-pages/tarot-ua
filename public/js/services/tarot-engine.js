...
export function getCards(limit = 78) {
  return TAROT_SEED.slice(0, limit);
}

export function getSpreadDefinitions() {
...
export function getCardOfDay(date = new Date()) {
...
function buildEnergy(majorCount, reversedCount, uprightCount, dominantSuit) {
  ...
  const intensity = majorCount >= 2 ? 'висока інтенсивність, бо старші аркани підсилюють значення всього розкладу' : 'м\'яка інтенсивність, де результат більше залежить від щоденних виборів';
...
function buildInteractions(spread, tone) {
  ...
  for (let i = 0; i < spread.length - 1; i++) {
    const left = spread[i], right = spread[i + 1];
    const lk = left.card.keywords.slice(0, 2).join(' і ');
    const rk = right.card.keywords.slice(0, 2).join(' і ');
    const tension = (left.reversed || right.reversed) ? 'створює напругу між заблокованою енергією та наступним кроком.' : 'підсилює плавний перехід від одного стану до іншого.';
    let connector;
    if (tone === 'mystic') connector = `наче поєднує ${lk} з ${rk}: це ${tension}`;
    else if (tone === 'practical') connector = `підказує практичний міст між темами «${lk}» та «${rk}»: це ${tension}`;
    else connector = `показує психологічний зв\'язок між «${lk}» та «${rk}»: це ${tension}`;
    pairs.push(`${left.position} → ${right.position}: «${left.card.name}» поруч із «${right.card.name}» ${connector}`);
  }
  if (spread.length >= 5) pairs.push(`Вісь розкладу: «${spread[0].card.name}» задає сенс, а «${spread[2].card.name}» показує, як цей сенс може проявитися в реальності.`);
  return pairs;
}
...
function buildNextStep(focus, adviceCard, tone) {
  ...
  if (tone === 'practical') return `Запиши один конкретний крок на найближчі 24 години: що ти зробиш із темою «${ak}», щоб розблокувати «${fk}».`;
  return `Поспостерігай, де тема «${fk}» викликає найбільшу реакцію, і обери м\'який крок у сторону «${ak}» без самотиску.`;
}

export function generateInterpretation(spread, spreadType, tone = 'psychological') {
  if (!spread.length) {
  ...
  const majorCount = spread.filter((i) => i.card.arcana === 'major').length;
  const suits = spread.reduce((acc, i) => { const k = i.card.suit || 'Старші аркани'; acc[k] = (acc[k] || 0) + 1; return acc; }, {});
